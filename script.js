(function(){
  ...
  const defaults = {name:"Tu Comercio",sub:"Rubro o lema",contact:"Dirección · Teléfono",footer:"Gracias por su consulta",prefix:"",next:1};
  ...
  function renderBiz(){
    ...
    const seqValue = (biz.prefix ? biz.prefix + "-" : "") + String(biz.next).padStart(4,"0");
    $("#seq").textContent = seqValue;
    $("#biz-seq-footer").textContent = "Presupuesto Nº " + seqValue; // 👈 pie de página
  }
  ...
  function newDoc(){
    $("#client").value="";$("#address").value="";$("#notes").value="";
    tbody.innerHTML="";for(let i=0;i<5;i++)addRow();
    biz.next++;LS.set("ps_biz",biz);renderBiz();$("#today").textContent=today();
  }
  ...
  $("#btn-pdf").addEventListener("click",()=>{
    ...
    const fileName="Presupuesto-"+document.querySelector("#seq").textContent+".pdf";
  ...
  $("#btn-settings").addEventListener("click",()=>{
    $("#s-name").value=biz.name;$("#s-sub").value=biz.sub;$("#s-contact").value=biz.contact;
    $("#s-footer").value=biz.footer;$("#s-prefix").value=biz.prefix;$("#s-next").value=biz.next;
    dlg.showModal();
  });
  $("#save-settings").addEventListener("click",e=>{
    e.preventDefault();
    biz.name=$("#s-name").value;biz.sub=$("#s-sub").value;biz.contact=$("#s-contact").value;
    biz.footer=$("#s-footer").value;biz.prefix=$("#s-prefix").value;biz.next=Number($("#s-next").value)||biz.next;
    LS.set("ps_biz",biz);renderBiz();dlg.close();
  });

  renderBiz();for(let i=0;i<5;i++)addRow();recalc();
})();
